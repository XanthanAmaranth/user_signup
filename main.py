...
def user_signup():
    ...
    if password == "":
        ...
        password_error = "Enter a password between 3 and 20 characters."
        password = ""
    elif " " in password:
    ...
    if password_confirmation == "" or password_confirmation != password:
        confirmation_error = "Password does not match."
        password_confirmation = ""
        
    if email != "":

        if len(email) < 3 or len(email) > 20:
            email_error="The email address must be between 3 and 30 characters."    
        elif not re.match(r"(^[a-zA-Z0-9_.+-]+@[a-zA-z0-9-]+\.[a-zA-Z0-9-.]+$)",email):
            email_error= "The email address must must contain a @, ., and no spaces."
